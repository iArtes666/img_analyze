...
Label(window, text='Необходимо выбрать коневую папку с пачками').grid(column=0, row=0)
btn_start = Button(window, text='Выбрать папку с пачками', command=start)
...
Checkbutton(window, text='Формировать pdf по пачкам', state='disabled').grid(column=0, row=3)

# add checkbox for search recursive_folder
recursive = BooleanVar(value=True)
Checkbutton(window, text='просмотр файлов в подкаталогах', var=recursive).grid(column=0, row=4)
...
Checkbutton(window, text='Расчет по алгоритму Дмитрия', var=dmitry_spec, state=dmitry_state).grid(column=0, row=5)

Label(window, text=f'dpi для анализа: ' + str(set_dpi)).grid(column=0, row=2)
...
Label(window, text='Статус:').grid(column=0, row=7)
style = ttk.Style()
